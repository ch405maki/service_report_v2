    ...
    <template #header>
      <div class="flex justify-between items-center">
        ...
        <div class="relative">
          ...
          <input type="text" id="table-search" v-model="searchQuery" class="block p-2 ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg w-80 bg-gray-50 focus:ring-gray-300 focus:border-gray-500" placeholder="Search Machine History">
        </div>
      </div>
    </template>
          ...
          <table class="w-full text-sm text-left rtl:text-right text-gray-500">
            <thead class="text-xs text-gray-700 uppercase bg-gray-200">
              <tr>
                ...
                <th scope="col" class="px-6 py-3 w-40">Department</th>
            ...
            <tbody>
              <tr v-for="report in filteredReports" :key="report.id" class="bg-white border-b border-gray-200">
                ...
                <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap w-40">
                  {{ report.staff?.department || 'No Department' }}
                </th>
                <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap w-40">
...
<script setup>
  ...
  const filteredReports = computed(() => {
    ...
    return props.reports.filter(report => {
      return report.machine?.machine_code?.toLowerCase().includes(query) ||
            report.staff?.name?.toLowerCase().includes(query);
    });
  });
</script>
